define("chart", [
  ...
  Chart.prototype = {
    ...
    drawJSON: function (el, data) {
      this.drawJSONMetricNav(el, data)

      // Chart container
      el.append("div")
        .attr("class", "chart-container")

      this.drawJSONRangeNav(el, data)

      // Legend container
      el.append("ul")
        .attr("class", "chart-legend")

      this.drawJSONChart(el, data)
    },


    drawJSONChart: function (el, data) {
      var self = this
      ...
      var minX = d3.min(oMetric.sources.map(function (source) {
        return d3.min(data[source.name + "_" + self.state.range].points, function (p) {
          return p.x
        })
      }))
      var maxX = d3.max(oMetric.sources.map(function (source) {
        return d3.max(data[source.name + "_" + self.state.range].points, function (p) {
          return p.x
        })
      }))
      ...
      oMetric.sources.forEach(function (source) {
        ...
        if (!source.hasOwnProperty("type") || source.type === "line")
          graph.append("path")
            .datum(data[source.name + "_" + self.state.range].points)
            .attr("class", "line")
            .style({
        ...
        if (!source.hasOwnProperty("showPoints") || source.showPoints !== false)
          graph.append("g")
            .attr("class", "points")
            .selectAll("circle")
            .data(data[source.name + "_" + self.state.range].points)
            .enter().append("circle")
            .attr("cx", function (d) {
              return x(d.x)
            })
    ...
    drawJSONRangeNav: function (el, data) {
      var self = this
      el.append("ul")
    ...
    drawJSONMetricNav: function (el, data) {
      var self = this
      el.append("ul")
