define("geomap", [
  ...
  "links",
  "loader"
], function (L, d3, linkColor, loadNodes) {
  "use strict"
  ...
  function projectPair(x) {
    var point = map.latLngToLayerPoint(new L.LatLng(x[0], x[1]))
    return [point.x, point.y]
  }

  function project(x) {
    return projectPair([x.latitude, x.longitude])
  }

  function update(data) {
    var nodes = Object.keys(data.nodes.nodes).map( function (k) {
      return data.nodes.nodes[k]
    })

    nodes = nodes.filter( function(d) {
      return d.nodeinfo.location != null
    })

    var links = data.graph.links.filter( function (d) {
      return d.target.node_id != null && d.source.node_id != null
    })

    links = links.map( function (d) {
      d.source = data.nodes.nodes[d.source.node_id]
      d.target = data.nodes.nodes[d.target.node_id]
      return d
    })

    links = links.filter( function(d) {
      return d.source.nodeinfo.location && d.target.nodeinfo.location && !d.vpn
    })

    var t = [
      d3.extent(nodes, function (d) { return d.nodeinfo.location.latitude }),
      d3.extent(nodes, function (d) { return d.nodeinfo.location.longitude })
    ]
    ...
    var nodesSvg = g.select(".nodes").selectAll(".node").data(nodes, function(d) { return d.nodeinfo.node_id })
    var linksSvg = g.select(".links").selectAll(".link").data(links)
    ...
    newNodes.append("circle")
            ...
            .attr("fill", function (d) {
              return d.flags.online ? (!d.nodeinfo.software ? "rgba(255, 55, 55, 1.0)" : "rgba(0, 255, 0, 0.8)") : "rgba(128, 128, 128, 0.2)"
            })
            .on("click", function(n) { window.location.href = "graph.html#" + n.nodeinfo.node_id })
            .append("title").text( function (n) { return n.nodeinfo.hostname ? n.nodeinfo.hostname : n.nodeinfo.node_id })
    ...
    linksSvg.style("stroke", function(d) {
          if (d.vpn) {
            return linkColor.default(d.tq)
          } else {
            return linkColor.wifi(d.tq)
          }
        })
        .append("title").text( function (d) {
          var a = new L.LatLng(d.source.nodeinfo.location.latitude,
                               d.source.nodeinfo.location.longitude)
          var b = new L.LatLng(d.target.nodeinfo.location.latitude,
                               d.target.nodeinfo.location.longitude)

          var distance = new Intl.NumberFormat("de-DE", {maximumFractionDigits: 0}).format(a.distanceTo(b))
    ...
    function reset() {
      var bottomLeft = projectPair(bounds[0]),
          topRight = projectPair(bounds[1])
      ...
      nodesSvg.attr("transform", function (d) { return "translate(" + project(d.nodeinfo.location).join(",") + ")"})

      linksSvg.attr("x1", function (d) { return project(d.source.nodeinfo.location)[0] })
               .attr("y1", function (d) { return project(d.source.nodeinfo.location)[1] })
               .attr("x2", function (d) { return project(d.target.nodeinfo.location)[0] })
               .attr("y2", function (d) { return project(d.target.nodeinfo.location)[1] })
  ...
  function positionLabels(nodes) {
    ...
    nodes.each( function (d) {
      var xy = project(d.nodeinfo.location)
      var label = {
    ...
    nodes.each( function (d) {
      var p = project(d.nodeinfo.location)
  ...
  //   var zoom = map.getZoom() / map.getMaxZoom()
    ...
    labels.forEach( function (d) {
      var t = d.nodes.map( function (d) { return project(d.nodeinfo.location) })
    ...
    */
    ...
    labelG.append("g").each( function (d) {
      ...
      d.nodes.forEach( function (n, i) {
        var name
        if (n.nodeinfo.hostname)
          name = n.nodeinfo.hostname
        else
          name = n.nodeinfo.node_id
