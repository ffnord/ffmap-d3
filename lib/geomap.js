...
function init() {
  ...
  var gpsbutton = d3.selectAll("#gpsbutton");

  var gpscoordinates = d3.select("header").append("span")
    .attr("id", "gpscoordinates")

  var remember_label = gpsbutton.text();

  gpsbutton.on("click",  function() {
    gpsbutton.text('klicke jetzt auf die Position auf der Karte');
    function clickhandler(e) {
      var latlng = e.latlng.lat.Math.round(6) + " " + e.latlng.lng.Math.round(6)
      gpscoordinates.html(' GPS: <input type="text" id="gps_out" value="'+latlng+'">');
      document.getElementById("gps_out").select();
      map.off("click", clickhandler);
      gpsbutton.text(remember_label);
    }
