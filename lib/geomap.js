...
function update(data) {
  ...
  if (query && query.length > 0) {
    ...
    for(var i = 0; i < queryParts.length; i++){
        var tmp = queryParts[i].split("=");
        params[tmp[0]] = decodeURIComponent(tmp[1]);
    }
  }

  if (params.lat && params.lat.length > 0 && params.lon && params.lon.length > 0) {
    var zoom = map.getMaxZoom();
    if (params.zoom && params.zoom < map.getMaxZoom() && params.zoom >= map.getMinZoom()) {
      zoom = params.zoom
    }
