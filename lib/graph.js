...
var settings_btn = d3.select("header")
                 .append("button")
                 .attr("class", "toggler")
...
meshinfo.append("input").attr("type", "range").attr("min", "0").attr("max", "1.0").attr("step", "0.01")
        .attr("value",ffmapConfig.friction)
        .on("change", function(d) {
...
meshinfo.append("input").attr("type", "range").attr("min", "0").attr("max", "2.0").attr("step", "0.01")
        .attr("value",ffmapConfig.strengh)
        .on("change", function(d) {
...
var toggleBtns = document.querySelectorAll("button.toggler");
for (var i=0; i<toggleBtns.length; i++) {
  toggleBtns[i].onclick = function() {
    var sidebar = d3.select("#sidebar .content")
    console.log(sidebar.attr("data-hide"));
    if (sidebar.attr("data-hide") === "hidden") {
      sidebar
        .attr("style", "")
        .attr("data-hide", "")
    } else {
