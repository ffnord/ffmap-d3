function nodetable(table, fn) {
  ...
  function prepare() {
    ...
    var params = parse_hash_parameters()
    name_ok = create_filter(params.name)
    firmware_ok = create_filter(params.firmware)
  }
  ...
  function update(data) {
    var non_clients = data.nodes.filter(function (d) {
      ...
      if (!name_ok(d.name ? d.name : d.id)) {
        return false
      }
      if (!firmware_ok(d.firmware)) {
        return false
      }
      return true
    })
